...
public class Agent_Method_1 {
	...
	public Agent_Method_1(int dim, boolean baseline) {
	...
	public ArrayList<Coordinate> getNextBestPoint_notreveal_driver(){
		ArrayList<Eqn> Original_CP=new ArrayList<Eqn>();
		ArrayList<Eqn> to_Feed_in=new ArrayList<Eqn>();
		ArrayList<Eqn> to_Change=new ArrayList<Eqn>();
		...
		for(Eqn c : to_Change) {
			c.sum=0;
			to_Feed_in.add(c);
			how_many_iter+=c.pts.size()+1;
		}
		ArrayList<Coordinate> results=getNextBestPoint_notreveal(to_Feed_in);
		for(int i = 0 ; i<how_many_iter;i++) {
			Iterator<Eqn> iter = to_Change.iterator();
			Eqn temp = iter.next();
			while(temp.sum>temp.pts.size()) {
				temp.sum=0;
				temp=iter.next();
			}
			temp.sum++;
			ArrayList<Coordinate> temp_results=getNextBestPoint_notreveal(to_Feed_in);
			for(Coordinate c : temp_results) {
				results.get(results.indexOf(c)).occur+=c.occur;
			}
		}
	...
	public ArrayList<Coordinate> getNextBestPoint_notreveal(ArrayList<Eqn> eqns) {//get Next Best Point Search Range: All KB
		//isConstrainted means whether we add the constraint on the remaining mine number
		SolutionSet ss = new SolutionSet(eqns);
...
//										"Sum=0, Singleton, adding to Safe and Covered" + mine[0] + "," + mine[1]);
					...
					if(c.pts.size()+c.sum==999||c.pts.size()==0) {
						iter_eqn.remove();
					}
				}
			}
			...
			if (totalNeighbors.size() - clueNum - safe.size() == hidden.size()) {
			 ...
			 for(int[] c : board.getNeighbors(x, y)) {
				 Coordinate c_int = new Coordinate(c[0],c[1]);
				 	...
				 	if(sum>900) {
				 		if(board.arr[c[0]][c[1]] == board.aSafeAndCovered
							|| (board.arr[c[0]][c[1]]>='0'&&board.arr[c[0]][c[1]]<='8')
							|| board.arr[c[0]][c[1]]==999) {
				 			sum--;//for all sum>900, this is a do not reveal point, we can just find all hidden nodes
				 		}
				 	}
